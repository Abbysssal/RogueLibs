...
:::note
When used, most items also play an animation with `UseItemAnim`:
:::
:::note
GIF needed
:::
:::note
You can safely use `Count` property. It automatically checks if the count is 0 and removes the item from the inventory if needed.
:::

## Inventory Checks {#inventory-checks}

:::tip
See [Inventory Checks](./inventory-events#inventory-checks) for more info.
:::

## `IDoUpdate` and `IDoFixedUpdate` {#update-interfaces}

If you want to update your items with Unity's `Update` or `FixedUpdate`, implement these interfaces:
...
A slightly more complicated food item. Uses `IDoUpdate` interface for cooldown.
